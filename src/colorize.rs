                ...
                if let Some(mesh) = meshes.get_mut(handle) {
                    ...
                    if let Some(VertexAttributeValues::Float32x3(positions)) =
                      mesh.attribute(Mesh::ATTRIBUTE_POSITION)
                     {
                         ...
                         let (mut max_x, mut max_y, mut max_z) = (-f32::INFINITY, -f32::INFINITY, -f32::INFINITY);
                         ...
                         println!("Mesh boundaries: [{min_x}:{max_x},{min_y}:{max_y},{min_z}:{max_z}]");

                         use crate::util::MapRange;
                         let colors: Vec<[f32; 4]> = positions
                             .iter()
                             .map(|[x, y, z]| { 
                                 let mapped = (x+y+z).map((min_x+min_y+min_z, max_x+max_y+max_z), (0.3, 0.9));
    ...
    )
{
     ...
     let mut m = Meshes{inner: HashSet::default()};
     for p in paths {
